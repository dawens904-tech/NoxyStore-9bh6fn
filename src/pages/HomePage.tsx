...
hello ai i add api it4gamer key now use server side and make all edit # Add Haiti Localization + Item4Gamer Integration

Hello AI,

Please implement a complete Haiti localization system that automatically switches between LootBar and Item4Gamer depending on the selected language and currency.
...
On mobile devices, display the real country flag and country name.
...
and clicks **Confirm**, do NOT instantly refresh the website.

Instead, start a silent migration process.
...
Show a full white loading page.
...
The loading speed should depend on the user's internet speed instead of using a fixed timer.

During this loading process:

* pause every LootBar API request
* stop all game API fetching
* stop checkout requests
* stop category loading
* stop product loading
* stop SKU loading

Everything should pause silently.

The user should never see broken data.

---

# 4. After Progress Reaches 100%

When the progress bar finishes:

Before refreshing the website:

Silently enable the Item4Gamer API.

Then perform one fast automatic refresh.

After refresh:

Everything should now use Item4Gamer instead of LootBar.

The transition should feel smooth without flashing or showing errors.

---

# 5. Haiti Behavior

When Haiti is selected:

Do everything automatically.

The user should NOT need to manually choose another provider.

Automatically:

* switch to French internally (silent only)
* keep Haiti selected
* keep currency HTG
* display prices in HTG
* use Item4Gamer only
* stop every LootBar API call

Only the backend provider changes.

The UI should continue working normally.

---

# 6. Return to Other Countries
...
show the same loading screen again.
...
* pause Item4Gamer API
* stop product fetching
* stop checkout
* stop categories

When finished:

Automatically return to LootBar.

Refresh once.

Everything should continue normally using LootBar.

---

# 7. Item4Gamer Authentication

Every Item4Gamer request must include the authentication header.
...
I will replace it later using an environment variable.

Never hardcode the real production key.

---

# 8. Item4Gamer API Endpoints

## Get Categories

GET

https://item4gamer.com/wp-json/reseller/v1/product/get-categories
...
https://item4gamer.com/wp-json/reseller/v1/product/get-products?category_id=89
...
https://item4gamer.com/wp-json/reseller/v1/product/get-product?product_id=103433
...
https://item4gamer.com/wp-json/reseller/v1/product/get-variation?variation_id=366068

---

# 9. Game Detail Page

When Haiti mode is active:

The Game Detail page must fetch products directly from Item4Gamer instead of LootBar.
...
Everything should come from Item4Gamer.

---

# 10. Checkout

On the Player ID / Checkout page,

replace the LootBar order system with Item4Gamer.
...
The dashboard and user order history should display the real Item4Gamer order status.
...
Inside **Manage Products**, add Item4Gamer support similar to LootBar.

Allow the admin to:

* import Item4Gamer products
* import categories
* import regions
* import variations
* import product images

The admin should also be able to edit:
...
The editing experience should match the current LootBar SKU editor.

---

# 14. Region Management

For every imported Item4Gamer product:
...
Display them inside the Manage Products page.

Each region should contain its own products and pricing.

---

# 15. Homepage

When Haiti mode is enabled:

The homepage should display only Item4Gamer games.

Categories should also come from Item4Gamer.

Do not display LootBar products while Haiti mode is active.

---

# 16. Automatic Provider Switching

Provider selection should happen automatically.
...
The user should never need to choose the provider manually.

Everything should switch silently in the background.
...
```env
ITEM4GAMER_API_KEY=DEMO_API_KEY
```

Use the environment variable everywhere in the application.
...
The website should support two completely functional providers:

* LootBar (default for all countries)
* Item4Gamer (automatic for Haiti)

Switching between providers should be seamless, using a white loading screen with a progress bar, silently pausing API requests during the transition. Haiti users should see only Item4Gamer products with prices in HTG, while all other countries continue using LootBar without interruption.
